...
http {
    ...
    server {
        listen 80;
        server_name {{ service['virtual_host'] }} {{ service['alt_virtual_host'] }};
    ...
    {% if service['alt_virtual_host'] %}
    # {{ service['alt_virtual_host'] }} - {{  service['id'] }} - HTTPS ssl-termination/ssl-bridging

    upstream upstream-https-{{ service['alt_virtual_host'] }} {
        server {{ service['service_name'] }}:{{ service['alt_service_port'] }};
    }
    {% endif -%}
    ...
    server {
        ...
        location / {
            ...
            proxy_pass $virtual_proto://upstream-https-$host;
        }

        {% if service['ws_virtual_host'] or service['ws_alt_virtual_host']%}
        location /socket.io/ {
            resolver 127.0.0.11;
            set $virtual_proto {{ service.virtual_proto }};
            proxy_pass $virtual_proto://upstream-$host;
    ...
    {% if service['alt_virtual_host'] %}
    # {{ service['alt_virtual_host'] }} - {{ service['service_id'] }} - HTTP
    upstream upstream-{{ service['alt_virtual_host'] }} {
        server {{ service['service_name'] }}:{{ service['alt_service_port'] }};
    }
    {% endif -%}
