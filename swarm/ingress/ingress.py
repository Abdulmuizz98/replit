...
while True:
    ...
    for service in services:
        ...
        virtual_host = ""
        ws_virtual_host = False
        virtual_proto = 'http'
        alt_virtual_host = ""
        ws_alt_virtual_host = False
        service_port = ""
        alt_service_port = ""
        service_name = ''
        ...
        if service['Spec'].get('Labels'):
            if service['Spec']['Labels'].get('ingress.host'):
                ...
                service_port = service['Spec']['Labels'].get('ingress.port')
                alt_service_port = service['Spec']['Labels'].get('ingress.alt_port')
            ...
            if service['Spec']['Labels'].get('ingress.ws_host'):
                ws_virtual_host = True
            
            if service['Spec']['Labels'].get('ingress.ws_alt_host'):
                ws_alt_virtual_host = True
        ...
        out = {
            ...
            'alt_service_port': alt_service_port,
            'service_name': service_name,
            'service_id': service_id,
            'ws_virtual_host': ws_virtual_host,
            'ws_alt_virtual_host': ws_alt_virtual_host
        }
    ...
    if current_nginx_config != new_nginx_config:
        ...
        print("After waiting for DNS service")
        
        # Reload nginx with the new configuration
        subprocess.call(['nginx', '-s', 'reload'])
        print("After reload of nginx")
